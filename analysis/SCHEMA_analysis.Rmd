...
LoF_intolerant_gene=unique(suppl_table11 %>% filter(pLI>0.9) %>% select(gene) %>%pull) 
LoF_intolerant_gene_id=suppl_table11 %>% filter(gene==LoF_intolerant_gene) %>% select(gene_id) %>% pull
...
{
  i=2
  cohort_ptv=SCHEMA_data %>% filter(consequence %in% PTV_definition==T & group==cohorts[i] & ac_case<=MAC_threshold & ac_ctrl<=MAC_threshold)
  cohort_syn=SCHEMA_data %>% filter(consequence== "synonymous_variant" & group==cohorts[i] & ac_case<=MAC_threshold & ac_ctrl<=MAC_threshold)
